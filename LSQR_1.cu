...
__global__ void kernel_get_b(float *d_b_tot, float *d_b, int i)
{
	int x = threadIdx.x;
...
void LSQR_1(float *A_mat, float *b, float *Vector, int *pos_x, int *pos_y, int n, int nz, int w, int h)
{
	int w_p = n*n;
	int h_p = nz;
	int h_b = n*n*h;
...
// Memory Allocation	
	...
	//6
	float *d_b_tot = NULL;
	cudaMalloc(&d_b_tot, sizeof(float) * h_b);
	cudaMemcpy(d_b_tot, b, sizeof(float) * h_b, cudaMemcpyHostToDevice);
	float *d_b = NULL;
	cudaMalloc(&d_b, sizeof(float) * h);
	float *d_Vector = NULL;
	cudaMalloc(&d_Vector, sizeof(float) * w);
	cublasSetVector(w, sizeof(float), Vector, 1, d_Vector, 1);
...
// Kernels
	...
	for (int i = 0; i < n*n; i++)
	{
		...
		kernel_get_b<<<grid6, block6>>> (d_b_tot, d_b, i);
		status = cublasSgemv(handle, CUBLAS_OP_T, h, w, &alpha, d_A_mat_shifted, h, d_b, 1, &beta, d_Vector, 1);
...
/*************************************************TEST**************************************************/
	// int *positions;
	// positions = (int *)malloc(sizeof(int) * w_p * h_p);

	// int *pos_newx;
	// pos_newx = (int *)malloc(sizeof(int) * n);

	// int *pos_newy;
	// pos_newy = (int *)malloc(sizeof(int) * n);

	// int *pos_newx_tot;
	// pos_newx_tot = (int *)malloc(sizeof(int) * n * n);

	// int *pos_newy_tot;
	// pos_newy_tot = (int *)malloc(sizeof(int) * n * n);

	// int *positions_shifted;
	// positions_shifted = (int *)malloc(sizeof(int) * n * n * nz);
	
	// int *layer_positions;
	// layer_positions = (int *)malloc(sizeof(int) *n *n);

	// float *b_small;
	// b_small = (float *)malloc(sizeof(float) * h);

	// ************************************************TEST*************************************************
	// cudaMemcpy(positions, d_positions, sizeof(int) * w_p * h_p, cudaMemcpyDeviceToHost);
	// cudaMemcpy(pos_newx, d_pos_newx, sizeof(int) * n, cudaMemcpyDeviceToHost);
	// cudaMemcpy(pos_newy, d_pos_newy, sizeof(int) * n, cudaMemcpyDeviceToHost);
	// cudaMemcpy(pos_newx_tot, d_pos_newx_tot, sizeof(int) * n * n, cudaMemcpyDeviceToHost);
	// cudaMemcpy(pos_newy_tot, d_pos_newy_tot, sizeof(int) * n * n, cudaMemcpyDeviceToHost);
	// cudaMemcpy(layer_positions, d_layer_positions, sizeof(int) * n * n, cudaMemcpyDeviceToHost);
	// cudaMemcpy(positions_shifted, d_positions_shifted, sizeof(int) * n * n * nz, cudaMemcpyDeviceToHost);
	// cudaMemcpy(A_mat_shifted, d_A_mat_shifted, sizeof(float) * w * h, cudaMemcpyDeviceToHost);
	// cudaMemcpy(b_small, d_b, sizeof(float) * h, cudaMemcpyDeviceToHost);
	cublasGetVector(w, sizeof(float), d_Vector, 1, Vector, 1);
