...
__global__ void kernel_divide_vector(float *d_u, float *d_b, int i)
{
	int x = threadIdx.x;
	d_b[x] = d_u[x + i * blockDim.x];

}

void LSQR_3(float *A_mat, float *u, float *Vector, int *pos_x, int *pos_y, int n, int nz, int w, int h)
{
	int w_p = n*n;
	int h_p = nz;
	int h_b = n*n*h;
...
// Memory Allocation	
	...
	cublasSetVector(h_b, sizeof(float), u, 1, d_u, 1);
	float *d_aux2 = NULL;
	cudaMalloc(&d_aux2, sizeof(float) * h);
	float *d_Vector2 = NULL;
	cudaMalloc(&d_Vector2, sizeof(float) * w);
	cublasSetVector(w, sizeof(float), Vector, 1, d_Vector2, 1);
	...
	for (int i = 0; i < n*n; i++)
	{
		...
		status = cublasSgemv(handle, CUBLAS_OP_T, h, w, &alpha_c, d_A_mat_shifted, h, d_aux2, 1, &beta_c, d_Vector2, 1);
   	...
   	cudaMemcpy(Vector, d_Vector2, sizeof(float) * w, cudaMemcpyDeviceToHost);
