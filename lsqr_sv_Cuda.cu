...
__global__ void kernel_shift_matrix(float *d_A_mat, float *d_layer, float *d_vec, float *d_aux1, int *d_positions_shifted, int w, int h)
{
    int x = threadIdx.x + blockDim.x * blockIdx.x;
    int y = threadIdx.y + blockDim.y * blockIdx.y;
    float sum = 0.0f;
    ...
    d_layer[x] = d_A_mat[y + (index - 1) * h];
    sum += d_layer[x] * d_vec[x]; 
    d_aux1[y] = sum;
...
__global__ void kernel_sum_vectors(float *d_v1, float *d_v2, int w)
{
    int x = threadIdx.x + blockDim.x * blockIdx.x;
    if (x >= w)
    {
...
// Memory Allocation    
    ...
    float *d_layer = NULL;
    cudaMalloc(&d_layer, sizeof(float) * w);
    //6
    float *d_b = NULL;
    ...
    thrust :: device_ptr<float> dev_ptr_v1(d_Vector1); 
    thrust :: fill(dev_ptr_v1, dev_ptr_v1 + h_b, 0);
    float *d_aux1 = NULL;
